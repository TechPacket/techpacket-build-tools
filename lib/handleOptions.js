...
async function handleBabel(options) {
    ...
    if (file) {
        ...
        logger.debug('babel config not found.');
        options.config.babelrc = false;
    }

    if (options.config.babelrc === false) {
        // Use our default preset when no babelrc was specified
        options.config.presets = [[require.resolve('@techpacket-build-tools/preset-babel-base'), { jsx: options.jsx }]];
    }
...
function handleHTML({ html, minimize, env }) {
    ...
    const defaultHtmlOption = Object.assign({ env }, inferHTML(html));
...
module.exports = async ({ options, command, env }) => {
    ...
    options.clearConsole = options.clearConsole || true;
    ...
    options.minimize = typeof options.minimize === 'boolean' ? options.minimize : command === 'build';
    options.html =
    ...
    options.css = {
        ...
        extract: typeof options.css.extract === 'boolean' ? options.css.extract : command === 'build',
        sourceMap:
