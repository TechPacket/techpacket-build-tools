...
module.exports = class WebpackLogger {
    ...
    apply(compiler) {
        if (compiler.hooks) {
            compiler.hooks.done.tap('WbpackReporter', this.handle.bind(this));
        } else {
    ...
    async displayErrors(stats, type = 'error') {
        ...
        const source = type === 'error' ? stats.compilation.errors : stats.compilation.warnings;

        if (type === 'error') {
            this.logger.error(chalk.red(`Failed to compile with ${source.length} errors\n`));
        } else if (type === 'warning') {
            this.logger.warn(chalk.yellow(`Compiled with ${source.length} warnings\n`));
        }
    ...
    displaySuccess(stats) {
        ...
        if (this.showCompiledIn) {
            this.logger.success(chalk.green(`Compiled successfully in ${stats.endTime - stats.startTime} ms!`));
        }
                ...
                '\n' +
                    textTable(
                        stats.toJson().assets.map(asset => {
                            return [
                                `${chalk.dim(
                                    `${path.relative(process.cwd(), stats.compilation.compiler.options.output.path)}/`,
